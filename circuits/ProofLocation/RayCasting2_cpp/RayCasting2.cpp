...
void RayCasting2_7_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void RayCasting2_7_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 27;}

uint get_total_signal_no() {return 12863;}

uint get_number_of_components() {return 159;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 12435;}
...
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 0)];
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 252],&lvar[3]); // line circom 32
Fr_band(&expaux[0],&expaux[1],&circuitConstants[2]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 252]); // line circom 38
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template/function " << myTemplateName << " line 38. " <<  "Followed trace of components: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
for (uint i = 0; i < 0; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
...
ctx->componentMemory[coffset].templateName = "RayCasting2";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 27;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[85]{0};
}
...
uint csoffset = mySignalStart+6281;
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
std::string new_cmp_name = "lessThanOrEqual"+ctx->generate_position_array(aux_dimensions, 1, i);
...
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
std::string new_cmp_name = "isLess"+ctx->generate_position_array(aux_dimensions, 1, i);
LessThan_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 257 ;
aux_cmp_num += 2;
...
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
std::string new_cmp_name = "and1"+ctx->generate_position_array(aux_dimensions, 1, i);
AND_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 3 ;
aux_cmp_num += 1;
}
...
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
std::string new_cmp_name = "and2"+ctx->generate_position_array(aux_dimensions, 1, i);
AND_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 3 ;
aux_cmp_num += 1;
...
uint csoffset = mySignalStart+9401;
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
...
uint csoffset = mySignalStart+9750;
uint aux_dimensions[1] = {12};
for (uint i = 0; i < 12; i++) {
std::string new_cmp_name = "processEdge"+ctx->generate_position_array(aux_dimensions, 1, i);
LessThan_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 257 ;
aux_cmp_num += 2;
...
uint csoffset = mySignalStart+9497;
for (uint i = 0; i < 1; i++) {
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[6]); // line circom 32
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[6]); // line circom 43
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[1],&lvar[1],&circuitConstants[2]); // line circom 44
Fr_mod(&expaux[0],&expaux[1],&circuitConstants[6]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 72);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[1])) + 1) + 4)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[2])) + 1) + 4)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
LessThan_1_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[2])) + 0) + 4)],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[1])) + 0) + 4)]); // line circom 58
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[2])) + 1) + 4)],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[1])) + 1) + 4)]); // line circom 59
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[1],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[1])) + 0) + 4)]); // line circom 60
Fr_mul(&expaux[0],&expaux[1],&lvar[4]); // line circom 60
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[1],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + (((2 * Fr_toInt(&lvar[1])) + 1) + 4)]); // line circom 61
Fr_mul(&expaux[0],&lvar[3],&expaux[1]); // line circom 61
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 36);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 48);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 36)]].signalStart + 0]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
AND_3_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 28)],&circuitConstants[2]); // line circom 72
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 3];
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 48)]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 72)]].signalStart + 0]); // line circom 73
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "rayCasting";
RayCasting2_7_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 12834 ;
aux_cmp_num += 158;
}
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 26],2);
}
// no need to run sub component
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 2],24);
}
// need to run sub component
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 24;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
RayCasting2_7_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
